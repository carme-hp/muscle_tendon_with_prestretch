...
k = nz-1 #fix tendon side of the muscle during prestretch

for j in range(ny):
    for i in range(nx):
      variables.prestretch_elasticity_dirichlet_bc[k*nx*ny + j*nx + i] = [0.0, 0.0, 0.0, None,None,None] # displacement ux uy uz, velocity vx vy vz

k = 0 #fix free side of the muscle
...
variables.prestretch_elasticity_neumann_bc = [{"element": k*mx*my + j*mx + i, "constantVector": variables.prestretch_bottom_traction_left, "face": "2-"} for j in range(my) for i in range(mx)]
  ...
  "Coupling": {
    ...
    "Term1": {
      "HyperelasticitySolver": {
        ...
        "residualNormLogFilename":    "out/"+variables.case_name + "/" + variables.scenario_name+"/log_prestretch_residual_norm.txt",   # log file where residual norm values of the nonlinear solver will be written
        ...
        "dirichletOutputFilename":    "out/"+variables.case_name + "/" + variables.scenario_name+"/prestretch_dirichlet_boundary_conditions",     # output filename for the dirichlet boundary conditions, set to "" to have no output
        ...
        "OutputWriter" : [
          ...
          # Paraview files
          {"format": "Paraview", "outputInterval": 1, "filename": "out/"+variables.case_name + "/" + variables.scenario_name+"/prestretch_u", "binary": True, "fixedFormat": False, "onlyNodalValues":True, "combineFiles":True, "fileNumbering": "incremental"},
        ],
        # 2. additional output writer that writes also the hydrostatic pressure
        "pressure": {   # output files for pressure function space (linear elements), contains pressure values, as well as displacements and velocities
          "OutputWriter" : [
            {"format": "Paraview", "outputInterval": 1, "filename": "out/"+variables.case_name + "/" + variables.scenario_name+"/prestretch_p", "binary": True, "fixedFormat": False, "onlyNodalValues":True, "combineFiles":True, "fileNumbering": "incremental"},
          ]
        },
        # 4. output writer for debugging, outputs files after each load increment, the geometry is not changed but u and v are written
        "LoadIncrements": {   
          "OutputWriter" : [
            # {"format": "Paraview", "outputInterval": 1, "filename": "out/"+variables.case_name + "/" + variables.scenario_name+"/prestretch_load_increments", "binary": True, "fixedFormat": False, "onlyNodalValues":True, "combineFiles":True, "fileNumbering": "incremental"},
          ]
        },
      }
    },
        ...
        "Term2": {
          "MuscleContractionSolver": {
                ...
                "DynamicHyperelasticitySolver": {
                  ...
                  "updateDirichletBoundaryConditionsFunction": update_dirichlet_bc,                  # muscle1_update_dirichlet_boundary_conditions_helper, function that updates the dirichlet BCs while the simulation is running
                  "updateDirichletBoundaryConditionsFunctionCallInterval": 1,         # every which step the update function should be called, 1 means every time step
